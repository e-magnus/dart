...
function updateGameState(newState) {
  ...
  if (isLegWin) {
    ...
    const prevLegs =
      prevState && prevState.players && prevState.players[winnerIndex]
        ? prevState.players[winnerIndex].legs
        : 0;
...
function updateScoreDisplay(playerIndex) {
  ...
  if (isBust) {
    console.log(
      `Bust detected for player ${playerIndex + 1}: score=${newScore}, darts ${oldDarts}->${newDarts}`
    );
...
function updateActiveOrWinnerIndicator(state) {
  ...
  const rows = state.players.map((_, index) =>
    document.getElementById(`player${index + 1}-section`)
  );
...
function triggerBustAnimation(playerIndex) {
  try {
    ...
    console.log(
      `Bust animation triggered for player ${playerIndex + 1} at (${relativeLeft}, ${relativeTop})`
    );
