...
function updatePlayerStatusDisplay() {
  ...
  if (!scoreElement || !gameState || !gameState.players) {
    return;
  }
  ...
  scoreElement.textContent = gameState.players.map(player => player.legs).join(' - ');
}
...
function showUndoRoundModal() {
  ...
  const lastPlayer = playerCount > 0 ? (activePlayerIndex - 1 + playerCount) % playerCount : 0;
...
function updatePlayerScores() {
  ...
  document.querySelectorAll('.player-card').forEach(card => {
    ...
    if (Number.isNaN(index) || !gameState.players[index]) {
      return;
    }
...
function renderPlayerCardsIfNeeded() {
  ...
  if (!container || !gameState || !gameState.players) {
    return;
  }
...
function updateDartTracker() {
  ...
  const totalTurns = Array.from(
    { length: playerCount },
    (_, index) => getRoundHistory(index).length
...
function syncInputsWithGameState() {
  ...
  if (firstToInput) {
    firstToInput.value = gameState.firstTo;
  }
...
function initPlayerNameEditing(state) {
  const container = document.getElementById('player-scores');
  if (!container || container.dataset.nameEditingBound === 'true') {
    return;
  }
  ...
  container.addEventListener('click', event => {
    ...
    if (Number.isNaN(playerIndex) || !currentState.players[playerIndex]) {
      return;
    }
    ...
    const saveName = () => {
      ...
      window.dispatchEvent(
        new CustomEvent('playerNameChanged', { detail: { playerIndex, name: newName } })
      );
    };
