...
function openNewGameModalWithCurrentSettings() {
  ...
  document.querySelector(
    `input[name="game-type"][value="${gameState.startScore || 501}"]`
  ).checked = true;
...
function openOverlay() {
  ...
  window.open(
    `/overlay/overlay.html?room=${currentRoomId}`,
    'overlay',
  ...
  ws.onmessage = event => {
    ...
    if (message.type === 'stateUpdate') {
      ...
      console.log(
        ...
        'Darts:',
        gameState.players[0].dartsThrown,
        'Avg:',
        gameState.players[0].average
      );
      console.log(
        'Player 1 - Score:',
        gameState.players[1].score,
        'Darts:',
        gameState.players[1].dartsThrown,
        'Avg:',
        gameState.players[1].average
      );
...
function updateUI() {
  ...
  document.getElementById(`p${inactivePlayerIndex + 1}-score`).textContent =
    gameState.players[inactivePlayerIndex].score;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document
    ...
    .addEventListener('click', openNewGameModalWithCurrentSettings);
